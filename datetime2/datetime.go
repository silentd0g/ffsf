...
func IsSameDay(t1, t2 int64) bool {
	tt1 := time.Unix(t1, 0).Local()
	tt2 := time.Unix(t2, 0).Local()
	y1, m1, d1 := tt1.Date()
	y2, m2, d2 := tt2.Date()
...
// 判断t2是不是t1的下一天
func IsNextDay(t1, t2 int64) bool {
	tt1 := time.Unix(t1, 0).Local()
	tt2 := time.Unix(t2, 0).Local()
...
// 判断t2是不是t1的下一周
func IsNextWeek(t1, t2 int64) bool {
	tt1 := time.Unix(t1, 0).Local()
	tt2 := time.Unix(t2, 0).Local()
	// 计算 t1 的下一周（加7天）
...
// 判断t2是不是t1的下一个月
func IsNextMonth(t1, t2 int64) bool {
	tt1 := time.Unix(t1, 0).Local()
	tt2 := time.Unix(t2, 0).Local()
	// 计算 t1 的下一个月
...
func GetDayOfMonth(t1 int64) int {
	tt1 := time.Unix(t1, 0).Local()
	_, _, day := tt1.Date()
	return day
}

func BeginTimeOfToday() int64 {
	now := time.Now().Local()
	y, m, d := now.Date()
