...
func (h *SSPacketHeader) To(b []byte) error {
	...
	pos += 4
	binary.BigEndian.PutUint32(b[pos:], h.CmdSeq)
	pos += 4
	binary.BigEndian.PutUint64(b[pos:], h.Extra)
	pos += 8
...
func (h *SSPacketHeader) From(b []byte) error {
	...
	pos += 4
	h.CmdSeq = binary.BigEndian.Uint32(b[pos:])
	pos += 4
	h.Extra = binary.BigEndian.Uint64(b[pos:])
	pos += 8
